...
public class FMIntroLoaderController : MonoBehaviour
{
    ...
    void OnLoginSuccess(LoginResult logResult)
    {
        ...
        }, result =>
        {
            ...
            PlayfabUtils.Instance.PrintPlayfabObject("[ON IntroLoader] title info ", result.AccountInfo.TitleInfo);
            FMClientSessionData.Instance.UserName = result.AccountInfo.TitleInfo.DisplayName;
            labLoading.text = "... Loading user info ...";
            Debug.Log("username loaded - " + FMClientSessionData.Instance.UserName);
            progress += 0.20f;
            total += progress;
                    ...
                    PlayfabUtils.Instance.GetTitleData(new List<string> { "fm_achievements", "fm_rewards" }, titleRes =>
                    {
                        ...
                        PlayFabClientAPI.GetCatalogItems(new GetCatalogItemsRequest(), catalogRes =>
                        {
                            ...
                            PlayfabUtils.Instance.GetUserReadOnlyData(new List<string>() { "fm_user_equipment" },
                                useDataRes =>
                                {
                                    //confirm if user_equipment is null or emtpy
                                    bool keyExists = useDataRes.Data.ContainsKey("fm_user_equipment");
                                    bool hasValidData = false;
                                    if (keyExists)
                                    {
                                        string rawValue = useDataRes.Data["fm_user_equipment"].Value;
                                        // Check if it's null, completely blank, or just empty JSON structural markup
                                        hasValidData = !string.IsNullOrEmpty(rawValue) &&
                                                    rawValue.Trim() != "{}" &&
                                                    rawValue.Trim() != "[]";
                                    }
                                    ...
                                    if (keyExists && hasValidData) //useDataRes.Data.ContainsKey("fm_user_equipment")
                                    {
